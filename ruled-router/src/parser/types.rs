...
// 实现所有基本数字类型
impl_from_to_param_for_numbers!(
...
impl FromParam for Version {
    fn from_param(param: &str) -> Result<Self, ParseError> {
        ...
        let major = parts[0].parse().map_err(|_| {
            ParseError::type_conversion(format!("Invalid major version: {}", parts[0]))
        })?;

        let minor = parts[1].parse().map_err(|_| {
            ParseError::type_conversion(format!("Invalid minor version: {}", parts[1]))
        })?;

        let patch = parts[2].parse().map_err(|_| {
            ParseError::type_conversion(format!("Invalid patch version: {}", parts[2]))
        })?;
...
mod tests {
    ...
    fn test_number_conversions() {
        ...
        assert_eq!(
            f64::from_param(&std::f64::consts::PI.to_string()).unwrap(),
            std::f64::consts::PI
        );
    ...
    fn test_vec_conversions() {
        ...
        assert_eq!(
            Vec::<String>::from_param("a,b,c").unwrap(),
            vec!["a", "b", "c"]
        );
    ...
    fn test_slug() {
        assert_eq!(
            Slug::from_param("hello-world_123").unwrap(),
            Slug("hello-world_123".to_string())
        );
    ...
    fn test_email() {
        ...
        assert_eq!(
            Email("test@example.com".to_string()).to_param(),
            "test@example.com"
        );
    }
