    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ParseError::SegmentCountMismatch { expected, actual } => {
                write!(
                    f,
                    "Path segment count mismatch: expected {expected} segments, found {actual}"
                )
            }
...
impl ParseError {
    ...
    /// 创建段内容不匹配错误
    pub fn segment_mismatch<S: Into<String>>(
        expected: S,
        actual: S,
        position: usize,
    ) -> Self {
...
mod tests {
    ...
    fn test_error_display() {
        ...
        assert_eq!(
            error.to_string(),
            "Path segment count mismatch: expected 3 segments, found 2"
        );
