...
use ruled_router_derive::{RouterData, RouterMatch, Query};
...
fn main() {
    ...
    let route_no_sub = UserRoute {
        id: 123,
        query: TestQuery { tab: Some("profile".to_string()) },
        sub_router: None,
    };
    ...
    let profile_route = ProfileRoute {
        query: TestQuery { tab: Some("basic".to_string()) },
    };
    ...
    let settings_route = SettingsRoute {
        query: TestQuery { tab: Some("general".to_string()) },
    };
