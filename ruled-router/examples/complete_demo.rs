  ...
  fn handle_request(url: &str) -> String {
    ...
    match route {
      ...
      AppRoute::Blog(blog_route) => {
        let mut response = format!("处理博客请求: 分类 = {}, 文章 = {}", blog_route.category, blog_route.slug);
...
mod tests {
  ...
  fn test_route_matching() {
    assert_eq!(RouteMatcher::match_route("/users/123"), AppRoute::User(UserRoute { id: 123 }));
    ...
    assert_eq!(RouteMatcher::match_route("/"), AppRoute::Home);

    assert_eq!(RouteMatcher::match_route("/invalid"), AppRoute::NotFound);
  }
