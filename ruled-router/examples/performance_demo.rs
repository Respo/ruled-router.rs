...
impl PerformanceTester {
    fn new() -> Self {
        let test_routes = (1..=1000)
            .map(|i| format!("/products/category{}/{}", i % 10, i))
            .collect();
    ...
    fn benchmark_route_formatting(&self) -> (u128, usize) {
        // 先解析所有路由
        let routes: Vec<ProductRoute> = self.test_routes
            .iter()
            .filter_map(|r| ProductRoute::parse(r).ok())
            .collect();
    ...
    fn benchmark_query_formatting(&self) -> (u128, usize) {
        // 先解析所有查询
        let queries: Vec<PaginationQuery> = self.test_queries
            .iter()
            .filter_map(|q| PaginationQuery::parse(q).ok())
            .collect();
    ...
    fn benchmark_roundtrip(&self) -> (u128, usize) {
        ...
        for (route_str, query_str) in self.test_routes.iter().zip(self.test_queries.iter()) {
            // 解析
            if let (Ok(route), Ok(query)) = (
                ProductRoute::parse(route_str),
                PaginationQuery::parse(query_str)
            ) {
                // 格式化
                ...
                // 重新解析
                if ProductRoute::parse(&formatted_route).is_ok() &&
                   PaginationQuery::parse(&formatted_query).is_ok() {
                    success_count += 1;
                }
            }
        }
