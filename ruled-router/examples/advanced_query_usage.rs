...
fn main() {
  ...
  let search_route = SearchRoute { version: "v1".to_string() };
  ...
  match parse_full_url::<SearchRoute, SearchQuery>(&full_url) {
    Ok((_reparsed_route, reparsed_query)) => {
...
mod tests {
  ...
  fn test_product_list_complex_query() {
    let url =
      "/api/v1/categories/electronics/products?brand=apple&brand=samsung&color=black&min_price=100.0&max_price=1000.0&in_stock=true";
