...
impl Router for ApiRoute {
    fn parse(path: &str) -> Result<Self, ParseError> {
        ...
        Err(ParseError::invalid_path(
            "No matching API route pattern".to_string(),
        ))
    }
...
fn main() {
    ...
    for url in urls {
        match FullRoute::parse(url) {
            Ok(full_route) => {
                println!(
                    "  {} -> API: {:?}, Query: {:?}",
                    url, full_route.api, full_route.query
                );
    ...
    url_formatter
        .query_formatter_mut()
        .set("q", "rust programming".to_string());
    url_formatter
        .query_formatter_mut()
        .add("tag", "tutorial".to_string());
    url_formatter
        .query_formatter_mut()
        .add("tag", "beginner".to_string());
    url_formatter
        .query_formatter_mut()
...
mod tests {
    ...
    fn test_search_query_parsing() {
        let query =
            SearchQuery::parse("q=rust&tag=tutorial&tag=beginner&role=admin&page=1&active=true")
                .unwrap();
